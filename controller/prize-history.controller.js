...
module.exports = {
    getHistoryController: async (req, res, next) => { 
        try {
            ...
            const sql = "SELECT `redeem_date`, `p`.`name`, `p`.`price`, `p`.`detail`, `p`.`cover` FROM `prize_history` AS `ph` JOIN `users` AS `u` ON `ph`.user_id = `u`.id JOIN `prizes` AS `p` ON `ph`.prize_id = `p`.id WHERE `u`.id = ?";
